...
def rate_finder(periods, installment_value, principal, final_amount, type=0, guess=0.10):
    """
    ...
    try:
        ...
        def financial_equation(rate):
            if rate == 0:
                return principal + installment_value * periods + fv
            else:
                return principal * (1 + rate)**periods + \
                       installment_value * (1 + rate * type) * (((1 + rate)**periods - 1) / rate) + final_amount
...
result_df = df \
    .withColumn("finance_interest_rate",
                rate_finder_udf(col("periods"), col("installment_value"), col("principal_value"), col("final_amount")))
result_df.show()
